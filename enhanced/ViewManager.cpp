...
{
	//receives first mouse movement and records it to calculate 
	//X and Y position offset for correct orientation 
	if (gFirstMouse)
	{
		gLastX = xMousePos;
		gLastY = yMousePos;
	...
	//calculate X offset and Y offset to move camera
	float xOffset = (xMousePos - gLastX);
	float yOffset = (gLastY - yMousePos);

	//set current positions to last position 
	gLastX = xMousePos;
	gLastY = yMousePos;
	//move camera to the calculated offset postions 
 ...
 *  ProcessScrollEvents()
 *
 *  This method is is called to process any scroll events that may be waiting in the 
 * event queue
 ***********************************************************/
...
void ViewManager::Scroll_Callback(GLFWwindow* window, double xoffset, double yoffset)
{
	if (g_pCamera != NULL)
	{
		//limit camera speed to a minimum of 0.1
		g_pCamera->MovementSpeed += yoffset * 0.1f;
		if (g_pCamera->MovementSpeed < 0.1f)
		{
			g_pCamera->MovementSpeed = 0.f;
		}
		//Capping camera movement speed to 60
		else if (g_pCamera->MovementSpeed > 60.0f)
		{
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	//if camera object is null, exit this method
	if (NULL == g_pCamera)
	{
		return;
	}

	//Keyboard Input for camera movment 
